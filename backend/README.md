...
1. Provision ApsaraDB Postgres in `ap-southeast-3` and create the `gingergig`
   database on it. See `docs/APSARADB_PROVISIONING.md` (Plan 07).
2. Add your dev machine's egress IP to the ApsaraDB allowlist.
3. `cp .env.example .env` and fill `DATABASE_URL`, `DATABASE_SSL_MODE`, `JWT_SECRET`.
   Generate `JWT_SECRET` with `python -c "import secrets; print(secrets.token_urlsafe(64))"`.
   The current ApsaraDB endpoint uses `DATABASE_SSL_MODE=disable`; switch to `require`
   only after SSL is enabled and verified on the server side.
...
| `make test` | Run pytest against the configured `DATABASE_URL` |
